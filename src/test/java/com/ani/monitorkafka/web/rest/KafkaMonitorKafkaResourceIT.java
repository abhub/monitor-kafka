...
import com.ani.monitorkafka.config.KafkaProperties;
...
class KafkaMonitorKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/kafka-monitor-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/kafka-monitor-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
